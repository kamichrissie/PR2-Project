...
void LineExtraction::extractLines(std::vector<Line>& lines) 
{
  ...
  ROS_DEBUG_STREAM("TEST:::" << c_data_.indices.size() << "\n");
  ...
  ROS_DEBUG_STREAM("Hello " << "LineExtraction - extractLines function");
  ...
  {
    ROS_DEBUG_STREAM("not enough points");
    return;
  }


  ROS_DEBUG_STREAM("Hello3 " << "\n");
  ...
  //Debugging
  std::vector<Line>::const_iterator cit = lines.begin();
  double startx = cit->getStart()[0];
  ROS_DEBUG_STREAM("Hello3 " << startx << "\n");
                                   ...
                                   const std::vector<unsigned int>& indices)
{
  ...
  ROS_DEBUG_STREAM(typeid(c_data_.bearings).name() << "\n");
